...
function requirePermission(permission, message = 'ليست لديك صلاحية لهذا الإجراء') {
...
function setupViewSwitcher() {
    ...
    // Start with production board as the default landing view
    Object.entries(views).forEach(([key, id]) => {
        const el = document.getElementById(id);
...
async function handleAction(e) {
    ...
    const actionPermissions = {
        ...
        'new-project': 'canCreateProjects',
        'save-project': 'canEditProjects',
        'clear-project': 'canCreateProjects',
        'export-project': 'canExportProjects',
        'import-project': 'canImportProjects',
        'import-project-file': 'canImportProjects',
        'do-import': 'canImportProjects',
        'duplicate-project': 'canCreateProjects',
        ...
        'cancel-edit-note': 'canEditProjects',
        'delete-spec-note': 'canEditProjects',
        'add-item': 'canEditProjects',
        'edit-item-modal': 'canEditProjects',
        'delete-item': 'canEditProjects',
        'update-item': 'canEditProjects',
        'generate-lpo': 'canEditProjects',
        'generate-full-lpo': 'canEditProjects',
        'print-lpo': 'canExportProjects',
        'print-full-report': 'canExportProjects',
        'print-specs-only': 'canExportProjects',
        'print-specs-with-designs': 'canExportProjects',
        'print-dashboard': 'canViewDashboard',
        'print-projects-table': 'canExportProjects',
        'upload-designs': 'canManageDesigns',
        'delete-design': 'canManageDesigns',
        'clear-designs-upload': 'canManageDesigns',
    ...
    if (action === 'delete-all' && !isCurrentOwner()) {
    ...
    switch (action) {
        ...
        case 'duplicate-project': Project.duplicate(target.dataset.projectId || target.closest('[data-project-id]')?.dataset.projectId); break;
        case 'delete-project': {
        ...
        case 'open-activity-log': {
            await ActivityLogPanel.open();
            break;
        }

        case 'refresh-activity-log': {
            await ActivityLogPanel.render();
            break;
        }
        ...
        list.innerHTML = logs.map((log) => `
            <article class="activity-log-item">
                <div class="activity-log-item__top">
                    <div class="activity-log-item__title">${escapeHtml(this.labelFor(log))}</div>
...
const AdminPanel = {
        ...
        if (summary) {
            ...
            summary.innerHTML = cards.map((item) => `
                <div class="admin-summary__card">
                    <span class="admin-summary__label">${item.label}</span>
                    <strong class="admin-summary__value">${item.value}</strong>
                </div>
            `).join('');
        }
    ...
    renderUserCard(user) {
        const isOwner = (user.email || '').toLowerCase() === OWNER_EMAIL;
        const permissions = { ...getRolePermissions(user.role), ...(user.permissions || {}) };
                ...
                <div class="admin-user-grid">
                    <div class="form-field">
                        ...
                        <select class="form-input form-input--select" data-admin-role ${isOwner ? 'disabled' : ''}>
                            ${['viewer', 'editor', 'manager', 'admin'].map((role) => `
                ...
                <div class="admin-user-permissions">
                    ${permissionFields.map(([key, label]) => `
                        <label class="permission-chip">
                            <input type="checkbox" data-admin-permission="${key}" ${permissions[key] ? 'checked' : ''} ${isOwner ? 'disabled' : ''} />
                            <span>${label}</span>
                        </label>
                    `).join('')}
                </div>
    ...
    async saveUser(uid) {
        ...
        await writeActivityLog('user_access_updated', 'user', uid, `تم تحديث صلاحيات المستخدم ${uid}`, {
            role,
            permissions,
        });
...
const _origProjectCreateNew = Project.createNew.bind(Project);
Project.createNew = function() {
    if (!requirePermission('canCreateProjects')) return;
    return _origProjectCreateNew();
};

const _origProjectSave = Project.save.bind(Project);
Project.save = async function() {
    if (!requirePermission('canEditProjects')) return;
    return _origProjectSave();
};

const _origProjectDuplicate = Project.duplicate.bind(Project);
Project.duplicate = async function(id) {
    if (!requirePermission('canCreateProjects')) return;
    return _origProjectDuplicate(id);
};

const _origProjectDeleteById = Project.deleteById.bind(Project);
Project.deleteById = async function(id) {
    if (!requirePermission('canDeleteProjects')) return;
    return _origProjectDeleteById(id);
};

const _origProjectDeleteAll = Project.deleteAll.bind(Project);
Project.deleteAll = async function() {
    if (!requirePermission('canDeleteProjects')) return;
    if (!isCurrentOwner()) {
        Toast.show('حذف جميع المشاريع متاح للمالك فقط', 'error');
        return;
    }
    return _origProjectDeleteAll();
};

const _origProjectExport = Project.export.bind(Project);
Project.export = function() {
    if (!requirePermission('canExportProjects')) return;
    return _origProjectExport();
};

const _origProjectImport = Project.import.bind(Project);
Project.import = function(jsonString) {
    if (!requirePermission('canImportProjects')) return false;
    return _origProjectImport(jsonString);
};

if (Project.importFromFile) {
    const _origProjectImportFromFile = Project.importFromFile.bind(Project);
    Project.importFromFile = async function(file) {
        if (!requirePermission('canImportProjects')) return false;
        return _origProjectImportFromFile(file);
    };
}

const _origDesignsUpload = Designs.upload.bind(Designs);
Designs.upload = async function() {
    if (!requirePermission('canManageDesigns')) return;
    return _origDesignsUpload();
};

const _origDesignsDelete = Designs.delete.bind(Designs);
Designs.delete = async function(id) {
    if (!requirePermission('canManageDesigns')) return;
    return _origDesignsDelete(id);
};
